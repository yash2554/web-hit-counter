...
$location = "<a href=\"javascript:window.open(\'https://db-ip.com/$ip\',\'$ip\',\'width=800,height=800\')\">$ip</a>";
$info ="<a href=\"javascript:window.open(\'http://www.ip-address.org/lookup/ip-locator.php?track=$ip\',\'$ip\',\'width=800,height=800\')\">Track $ip<a>";

if(!mysql_num_rows(mysql_query("SELECT ip_address FROM info WHERE ip_address = '$ip'"))) // check if the IP is in database
{
	// if not , add it.	
	$adddata = mysql_query("INSERT INTO $dbtableinfo (ip_address, user_agent, datetime, location, info) VALUES('$ip' , '$agent', '$datetime', '$location','$info')");
	if (!$adddata) 
	{
